...
public class SguRelatorioService {
    ...
    public SguRelatorioService(
            ...
            ObjectMapper mapper,
            @Value("${sgu.api.base-url}") String baseUrl,
            @Value("${sgu.api.key:}") String apiKey,
            @Value("${sgu.api.procedure-path:/api/procedure/p_prcssa_dados}") String procedurePath,
            @Value("${sgu.api.execution-path:/api/plapi}") String executionPath) {
        this.client = builder.baseUrl(removerBarraFinal(baseUrl)).build();
        this.mapper = mapper;
        this.apiKey = apiKey;
